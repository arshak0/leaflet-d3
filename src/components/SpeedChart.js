...
class SpeedChart extends React.Component {
  ...
  truck_count_func() {
    document.getElementsByClassName('truck_count_speed')[this.props.sensorId-1].innerHTML=this.props.sensorId
  }
  ...
  timer() {
    ...
    var new_speed=count[this.props.sensorId-1][this.state.currentCount]*3;

    document.getElementsByClassName('speed_speed')[this.props.sensorId-1].innerHTML=new_speed;

    if (count[this.props.sensorId-1][this.state.currentCount]>=120) {
